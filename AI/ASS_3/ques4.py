...
def iddfs_search(initial_state, goal_state):
    depth=0
    while True:
        visited=set()
        ...
        while stack:
            ...
            if current_state.blocks == goal_state.blocks:
                return current_state, current_depth

            if current_depth < depth:
                next_states = get_next_states(current_state)
                next_states = [state for state in next_states if str(state) not in visited]
                next_states = [(state, current_depth + 1) for state in next_states]
...
def main():
    ...
    # Perform DFS search
    final_state, depth = iddfs_search(initial_state, goal_state)
    ...
    if final_state:
        ...
        print('Depth of goal state:', depth)
    else:
